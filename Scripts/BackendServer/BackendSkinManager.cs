/*
플레이어의 스킨 보유 정보 관리하는 Singleton Manager

SkinData - 스킨 정보 관리하는 Class
...
그 외
- UpdateAllSkinData() : 모든 스킨 정보 서버에 업로드
- UpdateGlassesSkinData() : 안경 스킨 데이터 서버에 업로드
- UpdateHatSkinData() : 모자 스킨 데이터 서버에 업로드
- UpdateMaskSkinData() : 가면 스킨 데이터 서버에 업로드
*/
...
public partial class BackendSkinManager : MonoBehaviour {

    private static BackendSkinManager instance;
    public static BackendSkinManager Instance {
        get {
            return instance;
        }
    }
    ...
    // 업적 테이블 이름
    private const string SkinDataTableName = "Skin_List";

    // 현재 사용중인 업적 게임 정보의 inDate
    private string skinData_inDate;

    // 현재 사용중인 플레이어 업적 데이터(로컬)
    public SkinData skinData;

    // 데이터 불러오기를 통해 가져와진 업적 서버 데이터
    private JsonData skinLoadData;
    ...
    void RunLoadProcess() {
        ...
        switch (loadProcess) {
            ...
            case LoadProcess.FINISH:
                ...
                //다음씬 이동
                AddPercentage();
                DebugX.Log("Backend 스킨 정보 로드 완료!");

                DebugX.Log("로드 완료!");
                achievementBanner.SetInitPlayerSkin();
                break;
        }
    }
    ...
    private void GetPlayerData() {
        ...
        SendQueue.Enqueue(Backend.GameData.GetMyData, SkinDataTableName, new Where(), callback => {
            ...
            if (callback.IsSuccess()) {
                ...
                if (callback.FlattenRows().Count > 0) {
                    ...
                    skinLoadData = callback.FlattenRows();
    ...
    private void InitNewData() {
        ...
        //초기화된 정보 삽입
        SendQueue.Enqueue(Backend.GameData.Insert, SkinDataTableName, skinData.GetParam(), callback => {
            DebugX.Log("Player_Insert_NewData" + callback.ToString());

            if (callback.IsSuccess()) {
                //데이터 적용
                skinData_inDate = callback.GetInDate();
                LoadFinish();
            }
            else {
                RepeatThisProcess();
    ...
    private void SetServerDataToLocal() {
        ...
        bool isSuccess = skinData.SetData(skinLoadData[0]);
        if (isSuccess) {
    ...
    private void CheckNewInDateNeed() {
        ...
        string myLastInDate = skinLoadData[0]["inDate"].ToString();
        // UTC로 통일
    ...
    private void DeleteMyData() {
        ...
        //불러오는 데이터가 4개일 경우
        if (skinLoadData.Count == maxDataCount + 1) {

            string delete_indate = skinLoadData[maxDataCount]["inDate"].ToString();

            SendQueue.Enqueue(Backend.GameData.DeleteV2, SkinDataTableName, delete_indate, Backend.UserInDate, callback => {
        ...
        else if (skinLoadData.Count >= maxDataCount + 2) {
            List<TransactionValue> transactionList = new List<TransactionValue>();

            //트랜잭션은 최대 10개까지만 지원하므로 10개를 초과하면 에러가 발생할 수 있다.
            int transCount = 0;
            for (int i = maxDataCount; i < skinLoadData.Count; i++) {

                string inDate = skinLoadData[i]["inDate"].ToString();

                transactionList.Add(TransactionValue.SetDeleteV2(SkinDataTableName, inDate, Backend.UserInDate));
                transCount++;
    ...
    // 모든 스킨 데이터 서버에 업로드
    public void UpdateAllSkinData() {
        SendQueue.Enqueue(Backend.GameData.UpdateV2, SkinDataTableName, skinData_inDate, Backend.UserInDate, skinData.GetParam(), callback => {
            DebugX.Log("Player_Update AllSkinData" + callback.ToString());
    ...
    // 안경 스킨 데이터 서버에 업로드
    public void UpdateGlassesSkinData() {
        SendQueue.Enqueue(Backend.GameData.UpdateV2, SkinDataTableName, skinData_inDate, Backend.UserInDate, skinData.GetGlassListParam(), callback => {
            DebugX.Log("Player_Update SkinNameData" + callback.ToString());
    ...
    // 모자 스킨 데이터 서버에 업로드
    public void UpdateHatSkinData() {
        SendQueue.Enqueue(Backend.GameData.UpdateV2, SkinDataTableName, skinData_inDate, Backend.UserInDate, skinData.GetHatListParam(), callback => {
            DebugX.Log("Player_Update SkinNameData" + callback.ToString());
    ...
    // 가면 스킨 데이터 서버에 업로드
    public void UpdateMaskSkinData() {
        SendQueue.Enqueue(Backend.GameData.UpdateV2, SkinDataTableName, skinData_inDate, Backend.UserInDate, skinData.GetMaskListParam(), callback => {
            DebugX.Log("Player_Update SkinNameData" + callback.ToString());
    ...
    void OnApplicationQuit()
    {
        ...
        // 버닝비버 시연용 -> 서버에 업로드 되지 않도록
        // 종료 시 업적테이블의 모든 컬럼이 업데이트 되도록 한다.
